...
export default function Login(props) {
    ...
    const infoCheck={email,password}
      axios.post(`http://${IP}:5000/login`,infoCheck).then((res)=>{
          if(res.data==true){
    ...
    return (
        <>
          ...
          <ImageBackground source={img1} style={{flex:1,resizeMode:"contain",justifyContent:'center',width:'100%'}} >
        ...
        <View style={styles.btn}>
          <Button title='Enter' onPress={sendData} color={"#f4a460"}/>
        </View>
        ...
        <View style={styles.btn2}>
          <Button title='Home Page' onPress={goToHomePage} color={"#ffdab9"}/>
        </View>
        </ImageBackground>
